...
class PR2TorsoTest(MorseTestCase):
    ...
    def test_torso_translate(self):
        ...
        with Morse() as simu:
            ...
            self.assertAlmostEqual(getjoint(simu, joint), 0.0, 3)
            ...
            self.assertAlmostEqual(getjoint(simu, joint), 0.1, 1)

            simu.sleep(1)
            logger.info("Should be now at 0.2m...")
            self.assertAlmostEqual(getjoint(simu, joint), 0.2, 1)
            ...
            #simu.sleep(2)
            #self.assertFalse(action.running())
            #self.assertAlmostEqual(getjoint(simu, joint), 0.3, 1)
            ...
            simu.sleep(1.6)
            self.assertFalse(action.running())
            self.assertAlmostEqual(getjoint(simu, joint), 0.0, 1)
            ...
            logger.info("Should be now at 0.2m...")
            simu.sleep(.1)
            self.assertAlmostEqual(getjoint(simu, joint), 0.2, 3)

            # Test limits
            logger.info("Let's go as high as possible to check my limits...")
            simu.pr2.torso.set_translation(joint, 1)
            simu.sleep(.1)
            self.assertAlmostEqual(getjoint(simu, joint), 0.31, 3)
            simu.pr2.torso.set_translation(joint, -1)
            simu.sleep(.1)
            self.assertAlmostEqual(getjoint(simu, joint), 0.0, 3)
